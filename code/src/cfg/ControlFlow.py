...
class CFGNode(dict):
    ...
    def __init__(self, parents=[], ast=None):
        assert type(parents) is list  # if parent not list program break
        register_node(self)
...
class PyCFG:
    ...
    def on_for(self, node, myparents):
        init_node = CFGNode(parents=myparents, ast=ast.parse(
            '__iv = iter(%s)' % astor.to_source(node.iter).strip()).body[0])
        ast.copy_location(init_node.ast_node, node.iter)
        _test_node = CFGNode(parents=[init_node], ast=ast.parse(
            '_for: __iv.__length__hint__() > 0').body[0])
        ...
        extract_node = CFGNode(parents=test_node, ast=ast.parse(
            '%s = next(__iv)' % astor.to_source(node.target).strip()).body[0])
